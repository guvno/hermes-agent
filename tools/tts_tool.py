...
- Piper HTTP (local, free, no API key): Optional local Piper server returning WAV bytes
- MeloTTS (local, free, no API key): Optional isolated subprocess provider
...
PROVIDER_MAX_TEXT_LENGTH: Dict[str, int] = {
    ...
    "piper": 2000,        # local Piper/HTTP, conservative for multilingual quality
...
def _check_piper_package_available() -> bool:
...
def _generate_piper_local_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
...
def _check_piper_http_available(tts_config: Optional[Dict[str, Any]] = None) -> bool:
...
def _piper_uses_http(tts_config: Optional[Dict[str, Any]] = None) -> bool:
    """Return True when Piper is configured to use an external HTTP server."""
    config = tts_config or _load_tts_config()
    piper_config = config.get("piper", {}) if isinstance(config.get("piper"), dict) else {}
    return bool(piper_config.get("base_url"))


def _check_piper_available(tts_config: Optional[Dict[str, Any]] = None) -> bool:
    """Check availability for either native Piper or Piper HTTP mode."""
    config = tts_config or _load_tts_config()
    if _piper_uses_http(config):
        return _check_piper_http_available(config)
    return _check_piper_package_available()


def _generate_piper_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Generate Piper speech via native local Piper or configured HTTP mode."""
    if _piper_uses_http(tts_config):
        return _generate_piper_http_tts(text, output_path, tts_config)
    return _generate_piper_local_tts(text, output_path, tts_config)
...
def text_to_speech_tool(
    ...
    try:
        ...
        if command_provider_config is not None:
            ...
            piper_config = tts_config.get("piper", {}) if isinstance(tts_config.get("piper"), dict) else {}
            if piper_config.get("base_url"):
                logger.info("Generating speech with Piper HTTP TTS (local)...")
                piper_output = file_str
                # Piper HTTP returns WAV. If the caller requested .ogg, synthesize WAV first
        ...
        elif provider in ("edge", "neutts", "minimax", "xai", "kittentts", "piper", "melotts", "google_cloud") and not file_str.endswith(".ogg"):
            opus_path = _convert_to_opus(file_str)
            if opus_path:
                file_str = opus_path
...
if __name__ == "__main__":
    ...
    print(f"  Piper:      {'available' if _check_piper_available(config) else 'not installed/reachable'}")
    print(f"  MeloTTS:    {'available' if _check_melotts_available(config) else 'not configured'}")
