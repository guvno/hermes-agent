...
class TestPiperHttpCompatibility:
    def test_check_piper_available_uses_health_endpoint_when_base_url_configured(self, monkeypatch):
        calls = []
    ...
    def test_generate_piper_posts_text_and_writes_wav_when_base_url_configured(self, monkeypatch, tmp_path):
        calls = []
    ...
    def test_http_piper_requested_ogg_is_converted_to_telegram_opus(self, monkeypatch, tmp_path):
        ...
        cfg = {"provider": "piper", "piper": {"base_url": "http://127.0.0.1:5005"}}
        ...
        result = json.loads(tts_tool.text_to_speech_tool("안녕하세요", str(requested_ogg)))
...
class TestCheckTtsRequirementsPiper:
    def test_piper_install_satisfies_requirements(self, monkeypatch):
        ...
        monkeypatch.setattr(tts_tool, "_import_google_cloud_tts", lambda: (_ for _ in ()).throw(ImportError()))
        ...
        monkeypatch.setattr(tts_tool, "_check_melotts_available", lambda: False)
